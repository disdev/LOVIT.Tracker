...
public class WatcherService : IWatcherService
{
    ...
    public async Task NotifyWatchersAsync(Participant participant, Segment segment, Checkin checkin)
    {
        ...
        foreach (var watcher in watchers)
        {
            await _twilioService.SendMessageAsync(watcher.PhoneNumber, $"{participant.FullName} checked into {segment.ToCheckpoint.Name} at {segment.TotalDistance} miles at {checkin.When.ToLocalTime().ToShortTimeString()}. http://track.runlovit.com/participant/{participant.Bib}");
